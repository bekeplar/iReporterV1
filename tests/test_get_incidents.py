...
def test_get_a_red_flag(client):
    response = client.get(
        "api/v1/red-flags/10df0c67-5f2b-4e5d-8b45-7357bbf3bebb",
...
def test_get_a_red_flag_with_id_which_does_not_exist(client):
    response = client.get(
        "api/v1/red-flags/10df0c67-5f2b-4e5d-8b45-7357bbf7bebb",
        headers=user1_header,
    )
...
def test_get_all_interventions_with_expired_token(client):
    # test only logged in user get red flags
    response = client.get("api/v1/interventions", headers=expired_token_header)
    assert response.status_code == 401
    data = json.loads(response.data.decode())
    assert data == {"error": expired_token_message, "status": 401}
...
def test_get_all_interventions(client):
    response = client.get("api/v1/interventions", headers=user1_header)
    assert response.status_code == 200
    data = json.loads(response.data.decode())
    assert data["data"][0]["created_by"] == user1_id

    response = client.get("api/v1/interventions", headers=user2_header)
    assert response.status_code == 200
...
def test_get_a_intervention(client):
    response = client.get(
        "api/v1/interventions/79bb7006-272e-4e0c-8253-117305466b4a",
        headers=user1_header,
    )
    assert response.status_code == 200
    data = json.loads(response.data.decode())
