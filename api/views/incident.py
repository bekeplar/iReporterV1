...
create_redflag_bp = Blueprint("new_redflag", __name__, url_prefix="/api/v1")
create_intervention_bp = Blueprint("new_intervention", __name__, url_prefix="/api/v1")
...
@create_redflag_bp.route("/redflags", methods=["POST"])
@token_required
@non_admin
def new_redflag():
...
@create_intervention_bp.route("/interventions", methods=["POST"])
@token_required
@non_admin
